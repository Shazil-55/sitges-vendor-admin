...
export const TableComp: React.FC<TableProps> = ({
  ...
  const [PopOverBaseElement, setPopOverBaseElement] =
  ...
  return (
    <div className="table" style={{ width: "100%" }}>
      ...
      >
        <Table aria-label="customized table">
          ...
          <TableBody style={{ fontWeight: "400", fontSize: "14px" }}>
            ...
            {!isLoading &&
              data
                ?.slice(Page * rowsToShow, Page * rowsToShow + rowsToShow)
                .map((row, index) => {
                  return (
                    <TableRow
                      key={row.id || index}
                          ...
                          <TableCell key={index2} align={col.align || "left"}>
                            {col.Cell
                              ...
                              : col.accessor?.includes(".")
                                ? col.accessor
                                    .split(".")
                                    .reduce((acc, cur) => acc && acc[cur], row)
                                : row[col.accessor || ""]}
                          </TableCell>
                        );
                      })}
      ...
      >
        ...
        <Button
          disabled={
            Page === Math.ceil(data.length / rowsToShow) - 1 ||
